...
- **Matrix parent constraint breaks on mirrored components** — a mirrored component (the −180 rotation a right-side counterpart carries) makes the matrix parent constraint go haywire. Almost certainly the offset maths in `Matrix_functions.setMatrixParentWithOffset` rather than the constraint itself: an offset derived from a matrix carrying a 180° flip has a negative determinant on one axis, and decomposing that back into rotation gives a different-but-equivalent solution, so the offset no longer cancels the parent. Worth checking whether the mirrored guide positions themselves carry negative scale — the usual culprit — before touching the matrix code. Affects the Skinning tab's "matrix" constraint type and every component that parents through `setMatrixParentWithOffset`.
