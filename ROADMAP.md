...
- **Centralized debug-logging class** — **done, needs confirming in Maya.** New `ROSE_Debug/` package: `rose_log.py` (self-contained `ROSE_Log`, no stdlib logging, so nothing can collide with Maya's own logging config) and `rose_log_channels.py` (the channel list — adding one there is all that's needed for it to appear in preferences). Channels are dotted names resolved upward, so switching on `rose.skinning` covers everything under it; each logger caches its answer against a generation counter so a disabled check is a single integer compare. `debug()`/`info()` are gated per channel, `warning()`/`error()` always emit.

  All 78 per-module `*_DEBUG` flags are gone, along with the four spellings of `SERIALIZE_DEBUG` and the `REDU_DEBUG`/`VALIDATE_DEBUG` oddities — flags were mapped by concern, so a module with several flags now holds several loggers (e.g. `node_Editor_Scene.py` has `log`, `serialize_log`, `selection_log`, `build_log`). 302 one-line gated prints became `log.debug(...)`; 49 multi-line `if FLAG:` blocks became `if log.enabled:` guards so their loops stay lazy; the 206 previously ungated `print()` calls are now gated too — 198 as `debug`, 8 (all in `except` blocks or error paths) as `error`, so nothing user-facing was silenced.

  The `ROSEPreferences` stub is now a real window with a checkable channel tree. Ticking a branch cascades to its children and a partially-ticked branch drops out of the enabled set, so what the tree shows and what the logger resolves can never disagree. State persists in `QSettings("tlpf", "ROSE")` under `log_channels` and is restored before anything else in `rose_Editor.__init__`, so startup logging honours it.
