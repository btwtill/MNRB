...
- **Constraint system** — **reworked, needs confirming in Maya.** The mirrored-component bug turned out to be the target joint's `jointOrient`, which the matrix network never accounted for. Rather than patch it, constraints moved into `ROSE_Constraints/`: a `constraint` class alongside `deform`/`control`/`attribute`, a `ConstraintType` (matrix / native) per component set from the node properties, and a `ConstraintKind` (parent / point / orient / scale / aim) so a component says *what* it wants constrained and never *how*. Components call `self.constrain(child, parent, kind)`; a call site can override the type for one constraint by passing it, deliberately code-only. Switching the flag does not rebuild live — it flags the node with `needs_rebuild` (amber bar, not the red invalid bar, which would disable the very build button needed) and the user rebuilds.

  Four separate defects fixed on the way, all verified against a real Maya session with matrix and native agreeing to 0.000000:
  - **jointOrient ignored.** Maya keeps applying it with the channels at zero, so every matrix-constrained joint landed rotated by its own orient. The offset now cancels the neutral local matrix, and channel mode cancels it on the right so the rotation extracted is the one wanted.
  - **The child's own parent was ignored**, so a component whose input sat under a non-identity group was wrong before any mirroring came into it.
  - **`parentInverseMatrix` is a cycle.** Since Maya 2020 a node's `parentMatrix` *includes its own `offsetParentMatrix`*, so dividing the child's parent out with `child.parentInverseMatrix` feeds the driven plug back into itself. The DAG parent's `worldInverseMatrix` is used instead, and skipped entirely at the world root.
  - **`connectDecomposeToSRT` drove the child's `rotateOrder` from the decompose's unset input**, pinning every constrained object to XYZ. The decompose now reads the child's order instead.

  A fifth surfaced on the first Maya test: **`importlib.reload` rebuilds each `Enum` class as a new object**, and Enum members hash and compare by *identity* — so a module reloaded before its enum module keeps holding the old class, whose members compare unequal and miss every dict lookup against the new one. This raised `KeyError: <ConstraintKind.PARENT>` on the first build after a shelf Reload, and silently sent every attribute exposed by `rose_node_base` down the float branch. Fixed three ways: `ROSE_Data/rose_enum.py` holds an `enumValue()` helper, `constraint` and `attribute` remap incoming members through their `mapNameTo…` functions so stored state always belongs to the current class, the branches that must not fail compare on values, and the shelf reload list now loads the types before anything that imports them.

  The first Maya test also showed the migration had been **incomplete**: only `setMatrixParentWithOffset` call sites had moved across, while `setLiveMatrixParentNoOffset` (the deform-output → deform-joint link) and `setMatrixParentNoOffset` (control links, IK anchors) were still building their own `matrixParentLink` networks outside the class. `multi_deform`/`single_deform` hid this by calling `resetJointOrientations` on their deform joints; `simple_ik` does not, so its deform joints landed rotated by their own orient. All of those call sites now go through `self.constrain(..., maintain_offset = False)`, and `computeOffsetMatrix` cancels the neutral local matrix in the no-offset case too — "no offset" means landing *on* the driver, which without the cancellation means landing on it rotated by the child's own jointOrient. The two `resetJointOrientations` calls now run *before* the constraint rather than after, so they don't invalidate the offset it just computed. `connectDecomposeToSRT` (still used for component outputs) had the same reversed `rotateOrder` connection, now fixed.

  Migrating the control links surfaced one more thing: **a control's parent link can never be a native constraint.** A `parentConstraint` drives the child's translate/rotate, so the control stops being posable - the animator's pose is overwritten the next time anything upstream re-evaluates, which reads as the control snapping back whenever a parent control moves. Those links were previously hard-wired to `offsetParentMatrix` and so were never subject to the flag; being migrated made them obey it. They now pass `constraint_type = ConstraintType.MATRIX` explicitly (the code-level override), so switching a component to native affects its srt inputs, deform links and IK internals while the controls keep their channels free.

  Measured against a full four-component rig built with rotated guides: worst deform misorientation 8.98° → 0.000°, and of 110 objects in the rig exactly the 2 broken ones moved.

  Also: the offset goes into `multMatrix.matrixIn[0]` whole rather than through a `composeMatrix` built from translate/rotate/scale, which silently dropped shear; and the matrix network's DG nodes are now tracked and cleared on rebuild, where before they accumulated every time (deleting the component hierarchy does not take DG nodes with it). The Skinning tab's parallel native/matrix implementation now goes through the same class.
