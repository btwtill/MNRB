...
class NurbsShereOrientGuideShape(Serializable):
    def __init__(self, guide) -> None:
        ...
        self.name = self.guide.name + ROSE_Names.guide_orient_suffix
    ...
    def draw(self):
        ...
        direction_X_object = MC.createNurbsCone(self.name + ROSE_Names.guide_orient_suffix  + "_X", axis = [1, 0, 0], radius = 0.5)
        ...
        MC.assignObjectToShaderSet(direction_X_shape_node, ROSEColor.red.name + ROSE_Names.guide_shader_suffix)

        direction_Y_object = MC.createNurbsCone(self.name + ROSE_Names.guide_orient_suffix  + "_Y", axis = [0, 1, 0], radius = 0.5)
        ...
        MC.assignObjectToShaderSet(direction_Y_shape_node, ROSEColor.green.name + ROSE_Names.guide_shader_suffix)

        direction_Z_object = MC.createNurbsCone(self.name + ROSE_Names.guide_orient_suffix  + "_Z", axis = [0, 0, 1], radius = 0.5)
        ...
        MC.assignObjectToShaderSet(direction_Z_shape_node, ROSEColor.blue.name + ROSE_Names.guide_shader_suffix)
    ...
    def updateName(self, new_name):
        ...
        if MC.objectExists(self.name):
            ...
            new_auto_orient_blend_node_Name = self.auto_orient_blend_node.replace(old_name, new_name + ROSE_Names.guide_orient_suffix)
            self.auto_orient_blend_node = MC.renameObject(self.auto_orient_blend_node, new_auto_orient_blend_node_Name)
            new_auto_orient_input_node_Name = self.auto_orient_input_node.replace(old_name, new_name + ROSE_Names.guide_orient_suffix)
    ...
    def updateColor(self):
        if MC.objectExists(self.name):
            ...
            MC.assignObjectToShaderSet(shape_node, self.guide.color.name + ROSE_Names.guide_shader_suffix)
